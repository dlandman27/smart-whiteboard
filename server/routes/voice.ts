    ...
    for (let turn = 0; turn < 8; turn++) {
      const response = await anthropic.messages.create({
        ...
        system: [
          ...
          'For ALL task and to-do operations use the wiigit task tools (add_task, list_tasks, complete_task, list_task_lists, create_task_list) — NOT Notion. Tasks are stored in wiigit, not Notion. "Add to my tasks/todos/shopping list/groceries/any list" → add_task with the appropriate list_name. "What\'s on my list / read me my tasks" → list_tasks. "Mark X as done / complete X" → complete_task. The default list is "My Tasks". "Shopping list" → list_name "Shopping List", etc. If the user didn\'t say what to add, ask once then call add_task immediately with their answer.',
          'To show a task list as a widget on the board: use create_widget with widgetType "@whiteboard/task-list" and settings { listName: "Shopping List" } (or whatever list name). The user can say "show my shopping list on the board" or "put my tasks on the board" and you create that widget.',
          ...
          'quick_list_add is only for Notion-backed lists that are already saved in memory. Do NOT use it for personal tasks, todos, shopping lists, or any list the user manages day-to-day — use add_task for all of those.',
      ...
      if (response.stop_reason === 'tool_use') {
        ...
        for (const block of response.content) {
          if (block.type === 'tool_use') {
            const result = await executeVoiceTool(block.name, block.input as Record<string, any>, { notion, gcal, userId: req.userId! })
